...
class Project:
    ...
    def __init__(self, short_name, *args):
         ...
         * A project `short_name` attribute that will be used to pick only the samples
           that are associated to that particular project.
         * One or more <xlsx> files that describe samples locations and their metadata.
        ...
        self.all_dfs = [pandas.read_excel(str(path), header=1) for path in self.all_xlsx]
    ...
    def samples(self):
        ...
        # One Sample object per row #
        samples = [Sample(**dict(row)) for i,row in self.metadata.iterrows()]
